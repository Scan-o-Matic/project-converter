...
def assert_models_deeply_equal(a, b):
    if a is b:
        return
    elif type(a) != type(b):
        raise ValueError('Model not equal types: {} != {}'.format(type(a), type(b)))

    assert isinstance(a, Model) and isinstance(b, Model), (
        'Neither side is a model: {} & {}'.format(type(a).__name_, type(b).__name__)
    )
    ...
    for key in a.keys():
        ...
        if a_val != b_val:
            raise ValueError("Model not equal on '{}': {} != {}".format(key, a_val, b_val))
